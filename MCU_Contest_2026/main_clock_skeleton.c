...
uint8_t EEPROM_SaveAlarm(uint8_t hour, uint8_t min) {
    ...
    Timeout = 0;
    bI2C0_TxFIFO[0] = hour;
    ok = I2C0_Write(0, 1);            // ghi 1 byte tai dia chi 0
    if (!ok) return 0;

    for (volatile uint32_t d = 0; d < 20000; d++); // cho EEPROM ghi noi bo ~5ms

    Timeout = 0;
    bI2C0_TxFIFO[0] = min;
    ok = I2C0_Write(1, 1);            // ghi 1 byte tai dia chi 1
...
void Process_Key(uint8_t k) {
    ...
    if (k == KEY_SETUP && system_mode < MODE_EDIT_AL_HOUR) {
        ...
        if (system_mode == MODE_NORMAL) {
            ...
            alarm_armed = 1;
            EEPROM_SaveAlarm(alarm_hour, alarm_min);
            system_mode = MODE_NORMAL;
        }
    }
