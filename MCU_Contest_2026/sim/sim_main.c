...
{ /* level sampled as per-tick activity */
}
void Display_HW_LedOn(void)
{
    g_led = 0;
} /* active low: pin low = on  */
...
int main(void)
{
    ...
    CHECK(time_hour == 0 && time_min == 0 && time_sec == 0, "boot: time = 00:00:00");
    CHECK(alarm_hour == 0 && alarm_min == 0 && alarm_armed == 0, "boot: alarm defaults disarmed");
    ...
    CHECK(system_mode == MODE_NORMAL && time_hour == 0 && time_min == 56 && time_sec == 0,
          "edit: commit writes 00:56, seconds reset");
    ...
    CHECK(time_hour == 5 && time_min == 56, "timeout: clock frozen during time edit");
    ...
    CHECK(alarm_hour == 7 && alarm_min == 59 && alarm_armed == 1, "alarm: settings 07:59 armed");
    CHECK(EEPROM_ReadByte(0) == 0xA5 && EEPROM_ReadByte(1) == 7 && EEPROM_ReadByte(2) == 59 &&
              EEPROM_ReadByte(3) == 1 && EEPROM_ReadByte(4) == (uint8_t)(0xA5 ^ 7 ^ 59 ^ 1),
    ...
    time_hour = 7;
    time_min = 58;
    time_sec = 59; /* 1s before 07:59:00  */
    sim_tick(1500);
    ...
    time_sec = 58;
    sim_tick(3000);
    CHECK(time_hour == 0 && time_min == 0 && time_sec == 1, "clock: 23:59:59 wraps to 00:00:00");
    ...
    CHECK(system_mode == MODE_NORMAL && time_min == 59, "edit: commit after wrap works");
    ...
    CHECK(system_mode == MODE_NORMAL && alarm_hour == 0 && alarm_min == 59 && alarm_armed == 1,
          "alarm edit: commit persists wrapped alarm");
