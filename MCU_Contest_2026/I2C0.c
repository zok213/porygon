 ...
 * AUTHOR:     SA1
 * IC:         SN32F400
 * DESCRIPTION: I2C0 interrupt-driven driver (SONiX reference library).
 *
 * Hardware-verified changes (by the working board build):
 *   - PFPA selects SCL0 = P0.10, SDA0 = P0.11 (option 2) so the I2C pins do
 *     NOT collide with the 7-segment G/DP lines.
 *   - The old SysTick stop/start around each transaction was REMOVED: the
 *     driver runs fully interrupt-driven, so the display keeps refreshing
 *     during EEPROM access. The library does not self-set Timeout, so the
 *     application supplies an I2C hang watchdog (see eeprom.c).
 *****************************************************************************/
...
static void (*__fptrI2C0_irq_handler[])(void) = {
    __i2c0_irq_handler_normal,     __i2c0_irq_handler_dma_mst_tx, __i2c0_irq_handler_dma_mst_rx,
    __i2c0_irq_handler_dma_slv_tx, __i2c0_irq_handler_dma_slv_rx,
};
...
void I2C0_Init(void)
{
    ...
    /* IMPORTANT: select SCL0 = P0.10, SDA0 = P0.11 (option 2) according to
     * the board schematic - must not collide with 7-seg G/DP pins. */
...
/* --- (DMA I2C functions kept from the reference library, unused by the
 *      clock application but left intact so the driver stays complete.) --- */
