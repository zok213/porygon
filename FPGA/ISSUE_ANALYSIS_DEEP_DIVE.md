...
| :--- | :--- | :--- | :--- |
| **Xung Nhịp & Khởi Tạo** | `gowin_pllvr.v` & `top_system.v` | **HOÀN THIỆN 100% (VERIFIED)** | Tổng hợp PLL 50MHz phần cứng; mạch Reset Synchronizer giữ 20ms lúc boot chờ PLL lock ổn định. |
| **Lọc Dội Phím Bắt Sườn** | `button_debounce.v` | **HOÀN THIỆN 100% (VERIFIED)** | 2 tầng D-FF chống Metastability + bộ tích phân 20ms ($1,000,000$ nhịp clock) + Edge Detector xuất xung 1-clock $20\text{ns}$. |
| **Điều Chế PWM LED** | `pwm_led_controller.v` | **HOÀN THIỆN 100% (VERIFIED)** | Sóng mang 1kHz ($50,000$ nhịp clock) không nhấp nháy; 2,000 nấc độ sáng siêu mịn cho chu kỳ thở đúng $2.000\text{s}$. |
| **Truyền Nối Tiếp UART** | `uart_tx_string.v` | **HOÀN THIỆN 100% (VERIFIED)** | Bộ chia Baud 434 nhịp (sai số $0.0064\%$); cơ chế chốt `mode_latched` an toàn chống méo chuỗi telemetry. |
| **Mô Phỏng Tăng Tốc** | `tb_top_system_v2.v` | **HOÀN THIỆN 100% (VERIFIED)** | Áp dụng `defparam PWM_FREQ = 50_000` co chu kỳ từ $2.0\text{s} \rightarrow 40\text{ms}$, 11/11 ca kiểm tra tự động PASS. |
...
#### 🛠️ Phương án xử lý phòng thủ trong `top_system.v`
...
#### 📍 Vị trí code
- `FPGA/src/button_debounce.v:15-45`
...
#### 🛠️ Phương án xử lý phòng thủ trong `button_debounce.v`
...
#### 📍 Vị trí code
- `FPGA/src/uart_tx_string.v:35-85`
...
#### 🛠️ Phương án xử lý phòng thủ trong `uart_tx_string.v`
...
#### 📍 Vị trí code
- `FPGA/src/uart_tx_string.v:18-25`
...
#### 📍 Vị trí code
- `FPGA/src/pwm_led_controller.v:25-70`
...
#### 🛠️ Phương án xử lý phòng thủ trong `pwm_led_controller.v`
...
#### 📍 Vị trí code
- `FPGA/sim/tb_top_system_v2.v:15-30`
...
Toàn bộ 6 lỗi thiết kế phần cứng và kiểm thử đã được xử lý triệt để trong mã nguồn Verilog RTL tại thư mục [`FPGA/src/`](FPGA/src/). Hệ thống đạt trạng thái hoàn thiện cao nhất, sẵn sàng 100% cho vòng chấm thi thực tế trên bo mạch Kiwi Nano 4K!
